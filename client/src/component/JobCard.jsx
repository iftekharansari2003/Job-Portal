...
const JobCard = ({ job }) => {
  ...
  return (
    <div className='border p-6 shadow rounded hover:shadow-md transition-shadow'>
      ...
      <div className='mt-4 flex gap-4 text-sm'>
        <button onClick={() => {
          ...
          scrollTo(0, 0);
        }} className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded text-white">Apply now</button>
        <button onClick={() => {
          navigate(`/apply-job/${job._id}`);
          scrollTo(0, 0);
        }} className="px-4 py-2 rounded  border border-gray-500 text-gray-500 hover:bg-gray-100">Learn more</button>
      </div>
    </div>
  )
}
