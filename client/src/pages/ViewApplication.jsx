...
const ViewApplication = () => {
  ...
  return applicants ? applicants.length === 0 ? (
    <div className='flex items-center justify-center h-[70vh]'>
      <p className='text-xl sm:text-2xl'>No Applicant Applied Yet</p>
    </div>
  ) : (
            ...
            {applicants.filter(item => item.jobId && item.userId).map((applicant, idx) => (
              <tr key={idx} className='text-gray-700'>
                ...
                <td className='py-4 px-4 border-b text-center flex items-center'>
                  <img src={applicant.userId.image} alt="" className='w-10 h-10 border-full mr-3 max-sm:hidden' />
                  <span className='mr-3 '>{applicant.userId.name}</span>
                </td>
                ...
                <td className='py-2 px-4 border-b'>
                  <a href={applicant.userId.resume} target='_blank' className='bg-blue-50 text-blue-400 px-3 py-1 inline-flex gap-2 items-center'>
